...
- (void)viewDidLoad {
    ...
    _freefireMenu = [[Khanhios alloc] init];
    [self addChildViewController:_freefireMenu];
    _freefireMenu.view.frame = self.view.bounds;
    [self.view addSubview:_freefireMenu.view];
    [_freefireMenu didMoveToParentViewController:self];
    _freefireMenu.view.hidden = YES; // Start hidden
    
    
    NSString *processNamePath = @"/Library/Application Support/Sapphire/target_process.txt";
    NSError *error = nil;
    ...
    if (processName && processName.length > 0) {
        ...
        [self setTargetProcessName:@"freefireth"];
    }
...
- (void)setupDrawingOverlay {
    if (_drawingOverlay) return;
    
    _drawingOverlay = [[HUDDrawingOverlay alloc] initWithFrame:self.view.bounds];
    _drawingOverlay.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    _drawingOverlay.userInteractionEnabled = NO;
    [self.view addSubview:_drawingOverlay];
    
    // 传递MemoryUtils
    if (_memoryUtils && _memoryUtils.isValid) {
        _drawingOverlay.memoryUtils = _memoryUtils;
        _drawingOverlay.baseAddr = _memoryUtils.baseAddress;
        [_drawingOverlay startDrawing];
    }
}

// Removed retryReadingProcessName as we set the target process name directly.
...
- (void)checkProcessStatus {
    ...
    if (newMemoryUtils && newMemoryUtils.isValid) {
        _drawingOverlay.memoryUtils = newMemoryUtils;
        _drawingOverlay.baseAddr = newMemoryUtils.baseAddress;
        [_drawingOverlay startDrawing];
    }
...
- (void)removeModMenu {
    if (_freefireMenu) {
        [_freefireMenu.view removeFromSuperview];
        [_freefireMenu removeFromParentViewController];
        _freefireMenu = nil;
    }
    if (_menuButton) {
...
- (void)createModMenu {
    ...
    // The Free Fire menu is managed by _freefireMenu.
...
- (void)createMenuHeader {
    ...
    NSArray *segmentTitles = @[@"PREPARE", @"DRAWNING", @"MENU", @"王者"];
    NSArray *symbolNames = @[@"eye.fill", @"scope", @"bolt.fill", @"crown.fill"];
    CGFloat segmentWidth = 400.0 / 4.0;
...
- (NSArray *)getFunctionsForSegment:(NSInteger)segmentIndex {
    switch (segmentIndex) {
        ...
        case 3: // 王者荣耀绘制设置
            return @[
                @{@"title": @"绘制开关", @"type": @"button"},
                @{@"title": @"射线", @"type": @"toggle"},
                @{@"title": @"方框", @"type": @"toggle"},
                @{@"title": @"头像", @"type": @"toggle"},
                @{@"title": @"小地图", @"type": @"toggle"},
                @{@"title": @"技能计时", @"type": @"toggle"},
                @{@"title": @"野怪计时", @"type": @"toggle"},
                @{@"title": @"地图X", @"type": @"slider", @"min": @"0", @"max": @"100", @"default": @"32"},
                @{@"title": @"地图Y", @"type": @"slider", @"min": @"0", @"max": @"100", @"default": @"2"},
                @{@"title": @"地图大小", @"type": @"slider", @"min": @"50", @"max": @"300", @"default": @"123"},
                @{@"title": @"技能X", @"type": @"slider", @"min": @"0", @"max": @"200", @"default": @"11"},
                @{@"title": @"技能Y", @"type": @"slider", @"min": @"0", @"max": @"100", @"default": @"27"}
            ];
...
- (void)toggleSwitchChanged:(UISwitch *)sender {
    ...
    if (sender.tag >= 0 && sender.tag < functions.count) {
        ...
        } else if ([title isEqualToString:@"射线"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isLineEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else if ([title isEqualToString:@"方框"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isBoxEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else if ([title isEqualToString:@"头像"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isAvatarEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else if ([title isEqualToString:@"小地图"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isMapEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else if ([title isEqualToString:@"技能计时"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isSkillEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else if ([title isEqualToString:@"野怪计时"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isMonsterEnabled = sender.isOn;
                [_drawingOverlay saveSettings];
            }
        } else {
...
- (void)buttonTapped:(UITapGestureRecognizer *)sender {
    ...
    if (buttonIndex >= 0 && buttonIndex < functions.count) {
        ...
        } else if ([title isEqualToString:@"绘制开关"]) {
            if (_drawingOverlay) {
                _drawingOverlay.isDrawEnabled = !_drawingOverlay.isDrawEnabled;
                [_drawingOverlay saveSettings];
                
                UILabel *buttonLabel = [buttonContainer viewWithTag:2000 + buttonIndex];
                if (buttonLabel) {
                    buttonLabel.text = _drawingOverlay.isDrawEnabled ? @"DISABLE" : @"ENABLE";
                }
                
                NSString *drawStatus = _drawingOverlay.isDrawEnabled ? @"enabled" : @"disabled";
                [self showCustomAlert:@"绘制开关" description:[NSString stringWithFormat:@"王者荣耀绘制已%@！", drawStatus]];
            }
        } else {
...
- (void)customSliderPanned:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        // === 王者荣耀滑块处理 ===
        if ([title isEqualToString:@"地图X"]) {
            if (_drawingOverlay) {
                _drawingOverlay.mapX = actualValue;
                [_drawingOverlay saveSettings];
            }
            [sender setTranslation:CGPointZero inView:sliderContainer];
            return;
        ...
        if ([title isEqualToString:@"地图Y"]) {
            if (_drawingOverlay) {
                _drawingOverlay.mapY = actualValue;
                [_drawingOverlay saveSettings];
            }
            [sender setTranslation:CGPointZero inView:sliderContainer];
            return;
        ...
        if ([title isEqualToString:@"地图大小"]) {
            if (_drawingOverlay) {
                _drawingOverlay.mapSize = actualValue;
                [_drawingOverlay saveSettings];
            }
            [sender setTranslation:CGPointZero inView:sliderContainer];
            return;
        ...
        if ([title isEqualToString:@"技能X"]) {
            if (_drawingOverlay) {
                _drawingOverlay.skillX = actualValue;
                [_drawingOverlay saveSettings];
            }
            [sender setTranslation:CGPointZero inView:sliderContainer];
            return;
        ...
        if ([title isEqualToString:@"技能Y"]) {
            if (_drawingOverlay) {
                _drawingOverlay.skillY = actualValue;
                [_drawingOverlay saveSettings];
            }
            [sender setTranslation:CGPointZero inView:sliderContainer];
            return;
...
- (void)dealloc {
    ...
    // 清理绘制覆盖层
    if (_drawingOverlay) {
        [_drawingOverlay stopDrawing];
        [_drawingOverlay removeFromSuperview];
        _drawingOverlay = nil;
    }
}
...
- (void)viewDidLayoutSubviews {
    ...
    // 更新绘制覆盖层尺寸
    if (_drawingOverlay) {
        _drawingOverlay.frame = self.view.bounds;
    }
}
...
- (void)resetSliderToDefault:(UIView *)sliderContainer withValue:(CGFloat)defaultValue {
    ...
    } else if ([title isEqualToString:@"地图Y"]) {
        if (_drawingOverlay) {
            _drawingOverlay.mapY = defaultValue;
            [_drawingOverlay saveSettings];
        }
    } else if ([title isEqualToString:@"地图大小"]) {
        if (_drawingOverlay) {
            _drawingOverlay.mapSize = defaultValue;
            [_drawingOverlay saveSettings];
        }
    } else if ([title isEqualToString:@"技能X"]) {
        if (_drawingOverlay) {
            _drawingOverlay.skillX = defaultValue;
            [_drawingOverlay saveSettings];
        }
    } else if ([title isEqualToString:@"技能Y"]) {
        if (_drawingOverlay) {
            _drawingOverlay.skillY = defaultValue;
            [_drawingOverlay saveSettings];
        }
    } else {
